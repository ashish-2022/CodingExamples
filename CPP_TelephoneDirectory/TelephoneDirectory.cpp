...
int main(){
  ...
  fstream file;
  struct stat buf;
  // Create new phone directory if not yet created.
  if(stat(PHONE_DATA,&buf) == -1)
  {
    cout << "\nCreating new phone directory...\n";
    ofstream phoneDataFile (PHONE_DATA, ios::app);
    phoneDataFile.close();
  }
  file.open(PHONE_DATA, ios::ate | ios::in | ios::out | ios::binary);
  char ch,nm[20],telno[6];
  int choice,found=0; int cnt=0;
  while(1){
    cout<<"\n*****Phone Book*****\n";
